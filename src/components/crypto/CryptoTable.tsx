...
export function CryptoTable({ limit = 100, showSearch = true, showFilters = true }: CryptoTableProps) {
  ...
  const fetchCryptoData = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch('/api/crypto/top100', {
  ...
  return (
    <div className="space-y-4">
      {/* Search and Filters */}
      {showSearch && (
        <div className="flex gap-4 items-center bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-lg border border-blue-200">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 border border-blue-300 rounded-lg bg-white shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
          <button
            onClick={fetchCryptoData}
            className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 font-semibold shadow-md transition-all duration-200"
            disabled={loading}
          >
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gradient-to-r from-gray-50 to-gray-100 border-b-2 border-gray-200">
            ...
            <TableHeaderCell className="font-bold text-gray-700">
              📅 ATH Date
            </TableHeaderCell>
            <TableHeaderCell 
              sortable 
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${
                      crypto.marketCapRank <= 3 ? 'bg-gradient-to-r from-yellow-400 to-yellow-600' :
                      crypto.marketCapRank <= 10 ? 'bg-gradient-to-r from-orange-400 to-orange-600' :
                      crypto.marketCapRank <= 25 ? 'bg-gradient-to-r from-purple-400 to-purple-600' :
                      crypto.marketCapRank <= 50 ? 'bg-gradient-to-r from-blue-400 to-blue-600' :
      ...
      {!loading && filteredAndSortedCryptos.length > 0 && (
        <div className="bg-gradient-to-r from-green-50 to-blue-50 p-4 rounded-lg border border-green-200 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="text-lg">📊</span>
